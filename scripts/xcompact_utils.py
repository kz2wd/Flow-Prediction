...
def load_snapshot(snapshot_index, dims, folder, mu_viscosity, y_0_1_distance):
    ...
    x_sample = np.stack([pressure_data, tau_x, tau_z], axis=0)
...
def get_snapshot_xy(simulation_folder: Path, input_file_path=None):
    ...
    for idx in tqdm.tqdm(indices):
        delayed_yx = delayed(load_snapshot)(idx, dims, folder, mu_viscosity, y_0_1_distance)

        x_da = da.from_delayed(delayed_yx[0], shape=(3, nx, 1, nz), dtype=np.float32)
        y_da = da.from_delayed(delayed_yx[1], shape=(3, nx, ny, nz), dtype=np.float32)
