...
class ScaleFilteringRenormalized(TransformBase):
    ...
    def to_training(self, ds):
        ...
        print("Normalizing dog...")
        mean = extra_comp.mean(axis=(0, 2, 3, 4))
        std = extra_comp.std(axis=(0, 2, 3, 4))

        mean = mean[None, :, None, None, None]
        std = std[None, :, None, None, None]
        normalized_extra_comp = (extra_comp - mean) / std

        enriched_array = np.concatenate([normalized_ds, normalized_extra_comp], axis=1)
